...
Where $\mathbf{f}_i$ is the internal force acting on grid node $i$, i.e., elastic restoring force generated by deformation. $m_i\mathbf{g}$ is gravity, which we will omit from now on since its update is straightforward.
